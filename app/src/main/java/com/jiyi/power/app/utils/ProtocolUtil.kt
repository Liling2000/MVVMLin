...
import com.jiyi.power.app.bean.ProtocolParsedData
...
object ProtocolUtil {
    ...
    fun buildWriteCommand(
        ...
        return buildCommand(
            commandType = CmdConstant.CommandType.WRITE,
    ...
    fun buildEventCommand(
        ...
        return buildCommand(
            commandType = CmdConstant.CommandType.EVENT,
            functionCode = functionCode,
            dataHex = dataHex
        )
    }
    ...
    private fun buildCommand(
        ...
        val frame = ByteArray(CmdConstant.MIN_FRAME_LENGTH + dataBytes.size)

        frame[CmdConstant.INDEX_FRAME_HEAD] = CmdConstant.FRAME_HEAD.toByte()
        frame[CmdConstant.INDEX_COMMAND_CODE] = commandCode.toByte()
        frame[CmdConstant.INDEX_FUNCTION_CODE] = functionCodeByte
        frame[CmdConstant.INDEX_DATA_LENGTH] = (dataLength and CmdConstant.BYTE_MASK).toByte()

        // 连续读取命令通常没有数据区，但 dataLengthOverride 会声明期望读取的长度；
        // 其他写入/事件命令则把 dataHex 的真实字节复制到数据区。
        dataBytes.copyInto(
            destination = frame, destinationOffset = CmdConstant.INDEX_DATA_START
        )

        // 校验和只覆盖命令码到数据区，帧头 AA 和帧尾 55 不参与计算。
        frame[frame.lastIndex - 1] = calcChecksum(frame) ?: return null
        frame[frame.lastIndex] = CmdConstant.FRAME_TAIL.toByte()
    ...
    private fun buildCommandCode(
        ...
        if (commandType !in CmdConstant.CommandType.READ..CmdConstant.CommandType.EVENT) {
            return null
        }
        ...
        if (isContinuous) {
            commandCode = commandCode or CmdConstant.CommandBit.CONTINUOUS
        }

        if (isBlock) {
            commandCode = commandCode or CmdConstant.CommandBit.BLOCK
        }
    ...
    fun calcChecksum(frame: ByteArray): Byte? {
        if (frame.size < CmdConstant.MIN_FRAME_LENGTH) return null

        var sum = 0
        val checksumIndex = frame.lastIndex - 1

        // 协议校验和为命令码、功能码、长度和数据区逐字节求和后取低 8 位。
        for (index in CmdConstant.INDEX_COMMAND_CODE until checksumIndex) {
            sum += frame[index].toPositiveInt()
        }

        return (sum and CmdConstant.BYTE_MASK).toByte()
    }
    ...
    fun parseResponse(responseHex: String): ProtocolParsedData? {
        ...
        if (frame.size < CmdConstant.MIN_FRAME_LENGTH) return null
        if (frame.first().toPositiveInt() != CmdConstant.FRAME_HEAD) return null
        if (frame.last().toPositiveInt() != CmdConstant.FRAME_TAIL) return null

        val commandCodeValue = frame[CmdConstant.INDEX_COMMAND_CODE].toPositiveInt()
        val functionCodeValue = frame[CmdConstant.INDEX_FUNCTION_CODE].toPositiveInt()
        val dataLengthExtend = commandCodeValue shr 6
        val dataLength =
            (dataLengthExtend shl 8) + frame[CmdConstant.INDEX_DATA_LENGTH].toPositiveInt()

        // 按协议长度字段反推完整帧长，必须和实际字节数完全一致，避免截断包或粘包被误解析。
        val expectedLength = CmdConstant.MIN_FRAME_LENGTH + dataLength
        ...
        val commandTypeValue = commandCodeValue and CmdConstant.CommandBit.TYPE_MASK
        val commandType = commandTypeValue.toCommandTypeString() ?: return null

        // 数据区长度由协议字段决定，不包含校验和和帧尾。
        val dataBytes = frame.copyOfRange(
            fromIndex = CmdConstant.INDEX_DATA_START,
            toIndex = CmdConstant.INDEX_DATA_START + dataLength
        )

        return ProtocolParsedData(
            commandCode = commandCodeValue.toHexByteString(),
            commandType = commandType,
            isBlock = commandCodeValue and CmdConstant.CommandBit.BLOCK != 0,
            isContinuous = commandCodeValue and CmdConstant.CommandBit.CONTINUOUS != 0,
    ...
    private fun Byte.toPositiveInt(): Int {
        return this.toInt() and CmdConstant.BYTE_MASK
    }

    private fun Int.toHexByteString(): String {
        return "%02X".format(Locale.US, this and CmdConstant.BYTE_MASK)
    }

    private fun Int.toCommandTypeString(): String? {
        return when (this) {
            CmdConstant.CommandType.READ -> "READ"
            CmdConstant.CommandType.WRITE -> "WRITE"
            CmdConstant.CommandType.RESPONSE -> "RESPONSE"
            CmdConstant.CommandType.EVENT -> "EVENT"
            else -> null
        }
    }
}