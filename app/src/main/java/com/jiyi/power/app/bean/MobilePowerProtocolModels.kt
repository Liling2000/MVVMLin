...
enum class OperationType(val text: String) {
    READ_ONLY("只读"), WRITE_ONLY("只写"), READ_WRITE("读/写"), WRITE("写"), READ("读"), EVENT("事件"), RESPONSE(
        "响应"
    ),
    RESERVED("预留")
}
...
sealed class Payload {
     ...
     */
    data class RegisterBlock(val snapshot: MobilePowerSnapshot) :
        Payload()
...
enum class FastChargeProtocol(val code: Int, val text: String) {
    IDLE(0x00, "IDLE"), PD(0x01, "PD"), QC(0x02, "QC"), SCP(0x03, "SCP"), UNKNOWN(-1, "UNKNOWN")
}
...
enum class DisplayMode(val code: Int, val text: String) {
    STANDARD(0x00, "标准模式"), TIME(0x01, "时间模式"), WEATHER(0x02, "天气模式"), LYRICS(
        0x03,
        "歌词模式"
    ),
    WECHAT(0x04, "微信模式"), IMAGE(0x05, "图片投影模式"), MOOD(0x06, "心情模式"), UNKNOWN(
        -1,
        "未知模式"
    )
}
