...
class DeviceSettingActivity : BaseActivity<ActivityDeviceSettingBinding>() {
    ...
    private fun renderStoredValues() = with(mBinding) {
        val deviceName = BleDeviceStore.getDevices()
            .firstOrNull { it.bluetoothSn.equals(deviceSn, ignoreCase = true) }
            ?.bluetoothName
            ?.takeIf { it.isNotBlank() }
            ?: getString(R.string.device_setting_default_name)
        rowDeviceName.setRightTextValue(deviceName)
    ...
    private fun showRenameDialog() {
        AppPopupManager.showRenameDevice(
            context = this,
            currentName = mBinding.rowDeviceName.getRightTextValue(),
            onConfirm = ::saveDeviceName,
        )
    }

    private fun saveDeviceName(name: String) {
        BleDeviceStore.renameDevice(deviceSn, name)
