...
import com.liling.ble.manager.BluetoothDataWriteManager
...
open class BaseViewModel : ViewModel(), IViewModel, DefaultLifecycleObserver {
    ...
    open fun onBleDataReceive(data: String?) {}

    /**
     * 发送指令到设备
     * @param cmdStr String
     */
    fun sendCmdData(sn: String?, cmdStr: String) {
        sn?.let {
            BluetoothDataWriteManager.writeData(it, cmdStr)
        }
    }
     ...
     * @param cmdStr String
     */
    fun sendCmdDataNoQueue(sn: String?, cmdStr: String) {
        sn?.let {
            BluetoothDataWriteManager.writeDataNoQueue(it, cmdStr)
        }
    }
}
