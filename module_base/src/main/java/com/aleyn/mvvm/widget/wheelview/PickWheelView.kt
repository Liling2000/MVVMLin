 ...
 * @author zyyoona7
 */
open class PickWheelView @JvmOverloads constructor(
     ...
     * 是否绘制选中区域
     */
    var isSelectedTextFakeBold: Boolean = false
        set(value) {
            if (value == field) {
                return
            }
...
//        soundHelper.release()
    ...
    private fun initAttrsAndDefault(context: Context, attrs: AttributeSet) {
        ...
        minTextSize = typedArray.getDimensionPixelSize(
            R.styleable.WheelView_wv_minTextSize, DEFAULT_MIN_TEXT_SIZE
        )
        textAlign = convertTextAlign(
            typedArray.getInt(
                R.styleable.WheelView_wv_textAlign, TEXT_ALIGN_CENTER
            )
        )
        val textPadding = typedArray.getDimensionPixelSize(
            R.styleable.WheelView_wv_textPadding, DEFAULT_TEXT_PADDING
        )
        val textPaddingLeft = typedArray.getDimensionPixelSize(
            R.styleable.WheelView_wv_textPaddingLeft, DEFAULT_TEXT_PADDING
        )
        val textPaddingRight = typedArray.getDimensionPixelSize(
            R.styleable.WheelView_wv_textPaddingRight, DEFAULT_TEXT_PADDING
        )
        if (textPadding > 0) {
        ...
        leftTextSize = typedArray.getDimensionPixelSize(
            R.styleable.WheelView_wv_leftTextSize, DEFAULT_TEXT_SIZE
        )
        rightTextSize = typedArray.getDimensionPixelSize(
            R.styleable.WheelView_wv_rightTextSize, DEFAULT_TEXT_SIZE
        )
        leftTextMarginRight = typedArray.getDimensionPixelSize(
            R.styleable.WheelView_wv_leftTextMarginRight, DEFAULT_TEXT_PADDING
        )
        rightTextMarginLeft = typedArray.getDimensionPixelSize(
            R.styleable.WheelView_wv_rightTextMarginLeft, DEFAULT_TEXT_PADDING
        )
        leftTextColor =
            typedArray.getColor(R.styleable.WheelView_wv_leftTextColor, DEFAULT_SELECTED_TEXT_COLOR)
        rightTextColor = typedArray.getColor(
            R.styleable.WheelView_wv_rightTextColor, DEFAULT_SELECTED_TEXT_COLOR
        )
        ...
        selectedTextColor = typedArray.getColor(
            R.styleable.WheelView_wv_selectedTextColor, DEFAULT_SELECTED_TEXT_COLOR
        )
        lineSpacing = typedArray.getDimensionPixelSize(
            R.styleable.WheelView_wv_lineSpacing, DEFAULT_LINE_SPACING
        )

        isSelectedTextFakeBold = typedArray.getBoolean(R.styleable.WheelView_wv_selectedTextFakeBold, false)
        ...
        dividerType = convertDividerType(
            typedArray.getInt(
                R.styleable.WheelView_wv_dividerType, DIVIDER_FILL
            )
        )
        dividerHeight = typedArray.getDimensionPixelSize(
            R.styleable.WheelView_wv_dividerHeight, DEFAULT_DIVIDER_HEIGHT
        )
        dividerColor =
        ...
        dividerPadding = typedArray.getDimensionPixelSize(
            R.styleable.WheelView_wv_dividerPadding, DEFAULT_TEXT_PADDING
        )
        ...
        curvedArcDirection = convertCurvedArcDirection(
            typedArray.getInt(
                R.styleable.WheelView_wv_curvedArcDirection, CURVED_ARC_DIRECTION_CENTER
            )
        )
        curvedArcDirectionFactor = typedArray.getFloat(
            R.styleable.WheelView_wv_curvedArcDirectionFactor, DEFAULT_CURVED_FACTOR
        )
        //折射偏移默认值
        refractRatio =
...
//            soundHelper.soundPlayVolume = 0.3f
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        var width: Int =
            mainTextMaxWidth + leftAndRightExtraWidth + textPaddingLeft + textPaddingRight + paddingLeft + paddingRight
        //根据偏移计算偏移造成的额外宽度
    ...
    private fun calculateTextRect() {
        ...
        mainTextRect.set(
            mainLeft, mainTop, mainLeft + mainTextMaxWidth, mainTop + mainTextHeight
        )
    ...
    private fun calculateItemHeight() {
        ...
        itemHeight =
            (mainTextPaint.fontMetrics.bottom - mainTextPaint.fontMetrics.top + lineSpacing).toInt()
    }
    ...
    private fun checkEllipseText(oriText: String?): String {
        ...
        return TextUtils.ellipsize(
            originText, mainTextPaint, mainTextMaxWidth.toFloat(), TextUtils.TruncateAt.END
        )?.toString() ?: ""
    }
    ...
    private fun drawNormalItem(
        ...
        if (abs(item2CenterOffsetY) <= 0) {
            ...
            mainTextPaint.isFakeBoldText = isSelectedTextFakeBold
            ...
            //绘制其他条目
            mainTextPaint.color = normalTextColor
            mainTextPaint.isFakeBoldText = false
            //缩小字体，实现折射效果
            val textSize = mainTextPaint.textSize
            mainTextPaint.textSize = textSize * refractRatio
            //isBoldForSelectedItem==true 改变字体
